...
%% set path
path_to_dataset = "./"
dataset = {'pix2pix_result'};
ext = {'*.jpg', '*.png', '*.bmp'};
num_set = length(dataset);
record_results_txt = ['PSNR_SSIM_Results_model.txt'];
results = fopen(fullfile(record_results_txt), 'wt');
filename_mat = "./p2p_result.mat"
...
for idx_set = 1:num_set
    ...
    for scale = scale_all
        ...
        for idx_ext = 1:length(ext)
            filepaths = cat(1, filepaths, dir(fullfile(path_to_dataset, dataset{idx_set}, ext{idx_ext})));
        end
        PSNR_all = zeros(1, length(filepaths));
        SSIM_all = zeros(1, length(filepaths));
        for idx_im = 1:length(filepaths)
            name_SR = filepaths(idx_im).name;
            name_HR = strrep(name_SR,'_fake_B.png','.png'); % Important to change depending on image name of SR
            im_HR = imread(fullfile('./datasets/MOUSE/HR/Test', name_HR));
            im_SR = imread(fullfile(path_to_dataset, dataset{idx_set}, name_SR));
        ...
        save(filename_mat,'PSNR_all','SSIM_all')
    end
end
