    ...
    for idx_set = 1:num_set
        ...
        for scale = scale_all
            ...
            for idx_ext = 1:length(ext)
                filepaths = cat(1, filepaths, dir(fullfile('./SR', 'BI' , 'RCAN' , 'p2p_denoise', ext{idx_ext})));
            end
            ...
            for idx_im = 1:length(filepaths)
                ...
                %name_SR = strrep(name_HR, '.png', '_fake_B.png');
                name_HR = strrep(name_SR,'_fake_B.png','.png');
                im_HR = imread(fullfile('./HR', 'RCAN_MOUSE', name_HR));
                im_SR = imread(fullfile('./SR', degradation, [methods{idx_method}], dataset{idx_set}, name_SR));
